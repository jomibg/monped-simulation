...
def main() -> None:
    ...
    st.caption("Minimal interface for a single simulation run.")
    ...
    with st.form("run_simulation_form"):
        ...
        random_threshold = st.number_input(
            "Probability of authorities prohibiting renovation (higher = more likely to prohibit)",
            min_value=0.0,
            max_value=1.0,
            value=0.9,
            step=0.01,
        ...
        behavioral_threshold = st.number_input(
            "Behavioral threshold (higher = more likely to renovate)",
            min_value=0.0,
            max_value=1.0,
            value=0.9,
            step=0.01,
    ...
    with col2:
        ...
        distribution = truncnorm.rvs(a, b, loc=attitude_mean, scale=attitude_std, size=1000)
    ...
    if st.session_state.merged_map_data is not None:
        ...
        st.subheader("Renovation year (map controlled by slider)")
